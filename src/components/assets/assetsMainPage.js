...
class AssetsMainPage extends Component {
  ...
  async componentDidMount() {
    try {
      ...
      this.dataCharts = [];
      this.buildDatasets(assets);
      this.setState({ loading: false, assets: assets });
    } catch (error) {
  ...
  toggleModalTrade = (asset_id, trade_id) => {
    ...
    assets.forEach((asset, i) => {
      ...
      this.dataCharts[0].datasets[0].data.push(Number(asset.balance.toFixed(2)));
      ...
      if (asset.group.group_a === undefined || asset.group.group_a.length === 0) { // If there no classification
        this.dataCharts[1].datasets[0].data[0] += asset.balance;
      } else { // For assets with a classification
        this.inserIntoDataset(asset.group.group_a, 1, asset.balance);
      }

      if (asset.group.group_b === undefined || asset.group.group_b.length === 0) {
        this.dataCharts[2].datasets[0].data[0] += asset.balance;
      } else {
        this.inserIntoDataset(asset.group.group_b, 2, asset.balance);
      }

      if (asset.group.group_c === undefined || asset.group.group_c.length === 0) {
        this.dataCharts[3].datasets[0].data[0] += asset.balance;
      } else {
        this.inserIntoDataset(asset.group.group_c, 3, asset.balance);
      }
    });
